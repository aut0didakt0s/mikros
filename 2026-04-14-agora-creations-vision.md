...
### 9.7 Phase G client builds on the Messages API, not Managed Agents (decided, §3.3 + §4.3)

**Decision.** The agora-creations client (Phase G web, Phase H mobile) calls Anthropic's Messages API directly with the end user's API key. It does NOT build on Claude Managed Agents. The session store, harness loop, MCP client, and credential vault are implemented inside the agora-creations client.

**Rationale.** Managed Agents bills LLM inference to the agent owner's Anthropic account — the platform becomes the Anthropic customer and must build a pass-through billing pipeline to recoup from end users. That directly contradicts §4.3's "the platform never pays for LLM inference" thesis. Messages API direct keeps the user as Anthropic's billing counterparty: agora-creations never touches money for inference, never builds a billing stack, never carries float. The engineering cost (rebuilding session + harness + MCP proxy primitives) is a one-time fixed cost; the billing-pipeline alternative is an ongoing operational and financial burden.
