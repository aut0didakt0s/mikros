...
description: Enforces anti-bloat rules. PROACTIVELY invoke this skill when writing or reviewing code. Reads anti-patterns.md and gotchas.md before any edit.
...
2. `references/gotchas.md` — known failure modes from prior sessions.

## The iron rule

**A task must fit in one context window. If it can't, split the task — don't compress the context.**

If you realize the task you were given exceeds one context window, stop and return an error. Do not attempt to finish.
...
If you hit a failure mode during execution — a pattern that led you astray, a rule that was too loose — before returning, append an entry to `references/gotchas.md` describing the failure and the rule you would have wanted. This is how the skill learns over time.
