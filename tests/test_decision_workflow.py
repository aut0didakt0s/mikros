"""End-to-end tests for mikros decision framework workflow."""

from tests.conftest import call_tool

STEPS = ["frame", "options", "tradeoffs", "stress_test", "decide", "document"]


class TestDecisionHappyPath:
    def test_happy_path(self):
        r = call_tool("start_workflow", {"workflow_type": "decision", "context": "choose a database"})
        ...
        for i, step_id in enumerate(STEPS):
            r = call_tool("submit_step", {"session_id": sid, "step_id": step_id, "content": f"decision-{step_id}"})
...
class TestDecisionDirectiveContent:
    def test_directives_have_do_not(self):
        r = call_tool("start_workflow", {"workflow_type": "decision", "context": "test"})
